...
from DATA_G2 import GenDEBRIS, bands_mean, bands_std, RandomRotationTransform , class_distr, gen_weights
...
def main(options):
    ...
    in_channels=25,                  # model input channels (1 for gray-scale images, 3 for RGB, etc.)
    ...
    if options['agg_to_water']:
        class_distr = class_distr # Density of Mixed Water, Wakes, Cloud Shadows, Waves

    # Weighted Cross Entropy Loss & adam optimizer
    weight = gen_weights(class_distr, c = options['weight_param'])

    criterion = smp.losses.DiceLoss(ignore_index=-1,mode='multiclass')
    ...
    if options['mode']=='train':
        ...
        for epoch in range(start, epochs+1):
            ...
            for (image, target) in tqdm(train_loader, desc="training"):
                ...
                torch.save(logits, 'output_logit_exp.pt')
                torch.save(target, 'target_mask.pt')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--input_channels', default=25, type=int, help='Number of input bands')
    parser.add_argument('--output_channels', default=8, type=int, help='Number of output classes')
